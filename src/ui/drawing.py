...
def draw_fps_on_frame(frame: np.ndarray, fps: float) -> None:
    """
...
def draw_trajectory(frame: np.ndarray, trajectory: List[Tuple[int, int]]) -> None:
    """
    ...
    for i in range(1, trajectory_length):
        ...
        logger.debug(
            "Drew trajectory segment from %s to %s", trajectory[i - 1], trajectory[i]
        )
